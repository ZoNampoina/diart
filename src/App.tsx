...
function RecueilsPage({songs,entryMode,onImport,onComplete,onViewImported,onEditImported,toast}:{songs:Song[];entryMode:'tononkira'|null;onImport:(draft:SongDraft)=>Promise<Song>;onComplete:(id:string,patch:Partial<SongDraft>)=>Promise<void>;onViewImported:(song:Song)=>void;onEditImported:(song:Song)=>void;toast:(s:string)=>void}) {
  ...
  {importedSong&&<Modal className="post-import-modal" title="Import terminé" onClose={()=>setImportedSong(null)}><div className="post-import-success"><Check/><div><b>{importedSong.title}</b><span>{importedSong.artist||'Artiste non renseigné'} a été ajouté à DI’ART.</span></div></div><p>Que voulez-vous faire maintenant ?</p><div className="post-import-actions"><button className="primary" onClick={()=>{const s=importedSong;setImportedSong(null);onViewImported(s)}}><BookOpen/>Voir</button><button className="secondary" onClick={()=>{const s=importedSong;setImportedSong(null);onEditImported(s)}}><Pencil/>Modifier</button><button className="secondary" onClick={()=>{setImportedSong(null);setTononkiraTitle('');setTononkiraArtist('');setTononkiraResults([]);setTononkiraSearched(false);requestAnimationFrame(()=>tononkiraTitleRef.current?.focus())}}><Import/>Nouvel import</button></div></Modal>}
...
function SetlistStage({mode,list,songs,refresh,toast,onClose,onOpenSong,standalone=false}:{mode:'rehearsal'|'live';list:Setlist;songs:Song[];refresh:()=>Promise<void>;toast:(s:string)=>void;onClose:()=>void;onOpenSong:(s:Song)=>void;standalone?:boolean}) {
  ...
  return createPortal(<div className={'stage-mode '+mode} style={stageStyle}>
    ...
    {!standalone&&<footer className="stage-nav compact-stage-nav" style={{zIndex:5,background:'rgba(2,9,12,.97)',borderTop:'1px solid #17323a'}}>
