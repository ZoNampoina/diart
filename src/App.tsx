import { useDeferredValue, useEffect, useMemo, useRef, useState, type ReactNode, type RefObject, type FormEvent, type CSSProperties, type TouchEvent } from 'react'
...
function App() {
  ...
  const [songBack,setSongBack]=useState<{page:Page;label:string;stageMode?:'rehearsal'|'live'}>({page:'library',label:'Bibliothèque'})
  ...
  useEffect(()=>{
    const openRequestedSetlist=(event:Event)=>{
    ...
    window.addEventListener('diart:open-setlist',openRequestedSetlist)
    return()=>window.removeEventListener('diart:open-setlist',openRequestedSetlist)
  },[])
  ...
  const writeSongField=async(song:Song,field:QuickDeleteField,value:unknown)=>{
    await updateSong(song.id,{[field]:value} as Partial<SongDraft>)
    const updatedAt=new Date().toISOString()
    patchLocal(song.id,{[field]:value,updatedAt} as Partial<Song>)
    setSelected(prev=>prev?.id===song.id?{...prev,[field]:value,updatedAt}:prev)
  }
  const quickDeleteSongField=async(song:Song,field:QuickDeleteField)=>{
    const previous=(song as unknown as Record<string,unknown>)[field]
    const label=QUICK_DELETE_LABELS[field]
    const snapshot=previous&&typeof previous==='object'?JSON.parse(JSON.stringify(previous)):previous
    await writeSongField(song,field,quickDeleteEmptyValue(field))
    const activity=await logActivity('delete',label+' supprimé',`${label} supprimé depuis la fiche morceau`,{songId:song.id,songTitle:song.title,restoreData:{kind:'song_field',field,value:snapshot,label}})
    scheduleSync(200)
  ...
  const restoreDeletion=async(item:ActivityEntry):Promise<boolean>=>{
    ...
    if(item.restoreData?.kind==='song_field'){
      const field=item.restoreData.field as QuickDeleteField
      await writeSongField(song,field,item.restoreData.value)
      await markActivityRestored(item.id)
      await recordActivity('restore',item.restoreData.label+' restauré','Restauration depuis l’historique',{songId:song.id,songTitle:song.title})
      await refresh()
      scheduleSync(200)
      toast(item.restoreData.label+' restauré.')
      return true
    }
    if(song.deletedAt){
      await restoreSong(song.id)
      await markActivityRestored(item.id)
      await recordActivity('restore','Morceau restauré','Restauration depuis l’historique',{songId:song.id,songTitle:song.title})
      await refresh()
      scheduleSync(200)
      toast('Morceau restauré.')
      return true
    }
    toast('Cette suppression a déjà été restaurée.')
    return false
  }
  ...
  const openSong=(s:Song,origin?:{page:Page;label:string;stageMode?:'rehearsal'|'live';setlist?:Setlist})=>{
    ...
    const originSetlist=resolvedOrigin.page==='setlist'?(origin?.setlist??currentSetlist):null
    if(resolvedOrigin.page==='library')setLibraryScrollY(currentScrollY())
    if(originSetlist&&originSetlist.id!==selectedSetlistId)setSelectedSetlistId(originSetlist.id)
    setSongBack({page:resolvedOrigin.page,label:resolvedOrigin.label,stageMode:resolvedOrigin.stageMode})
    if(originSetlist){
      setSetlistSongKeyDraft({listId:originSetlist.id,songId:s.id,key:setlistSongDisplayKey(originSetlist,s)||s.originalKey||s.personalKey||''})
  ...
  return <div className="app-shell" onTouchStart={beginSidebarSwipe} onTouchMove={moveSidebarSwipe} onTouchEnd={endSidebarSwipe} onTouchCancel={()=>{sidebarSwipeRef.current=null}}>
    ...
    <main className="main">
      ...
      <div className={`content page-transition-surface page-${page}`} key={page+':'+selectedArtist+':'+selectedAuthor+':'+selectedSetlistId+':'+(selected?.id??'')}>
        ...
        {page==='history'&&<HistoryPage songs={songs} onRestore={restoreDeletion}/>} 
...
function QuickSetlistAdd({song,setlists,refresh,toast,suppressOpenSetlistId}:{song:Song;setlists:Setlist[];refresh:()=>Promise<void>;toast:(s:string,action?:Toast['action'])=>void;suppressOpenSetlistId?:string}) {
  ...
  const confirmAdd=()=>{
    ...
    setOpen(false)
    setPendingId('')
    void updateSetlist(pending.id,{songIds:ids}).then(()=>{
      void refresh()
      const message=`${song.title} ajouté à ${pending.name}.`
      if(suppressOpenSetlistId===pending.id)toast(message)
      else toast(message,{label:'Voir la setlist',run:()=>window.dispatchEvent(new CustomEvent('diart:open-setlist',{detail:{id:pending.id}}))})
    }).catch(()=>toast('Ajout à la setlist impossible.')).finally(()=>setBusy(false))
  }
...
function ArtistDetailPage({artist,songs,setlists,refreshSetlists,toast,onBack,onOpen,onFav,onAdd}:{artist:string;songs:Song[];setlists:Setlist[];refreshSetlists:()=>Promise<void>;toast:(s:string)=>void;onBack:()=>void;onOpen:(s:Song)=>void;onFav:(s:Song)=>void;onAdd:()=>void}) {
  const [q,setQ]=useState('')
  const deferredQ=useDeferredValue(q)
  const filtered=useMemo(()=>songs.filter(s=>searchSong(s,deferredQ)).sort((a,b)=>a.title.localeCompare(b.title,'fr',{sensitivity:'base'})),[songs,deferredQ])
...
function SongDetail({song,backLabel,setlists,refreshSetlists,refreshSongs,toast,suppressOpenSetlistId,onBack,onEdit,onQuickDelete,onArtist,onFav,onLyricsSave,onRecueilSearch,onDelete,setlistKey,onSetlistKey,setlistContext,onStageOpenSong}:{song:Song;backLabel:string;setlists:Setlist[];refreshSetlists:()=>Promise<void>;refreshSongs:()=>Promise<void>;toast:(s:string,action?:Toast['action'])=>void;suppressOpenSetlistId?:string;onBack:()=>void;onEdit:()=>void;onQuickDelete:(field:QuickDeleteField)=>void;onArtist:(name:string)=>void;onFav:()=>void;onLyricsSave:(lyrics:string)=>Promise<void>;onRecueilSearch:(prefill:{title:string;artist:string})=>void;onDelete:()=>void;setlistKey?:string;onSetlistKey?:(key:string)=>void;setlistContext?:{list:Setlist;songs:Song[];mode?:'rehearsal'|'live'};onStageOpenSong?:(song:Song,mode:'rehearsal'|'live')=>void}) {
  ...
  const [fullscreenMode,setFullscreenMode]=useState<'standalone'|'rehearsal'|'live'|null>(null)
  const [fullscreenChoiceOpen,setFullscreenChoiceOpen]=useState(false)
  ...
  <div className="musician-grid">
    ...
    {(song.notes||song.referenceUrl)&&<section className="panel notes-panel"><h2>Notes générales</h2>{song.notes&&<div className="notes-row-with-delete"><p className="notes">{song.notes}</p>{quickDelete('notes')}</div>}{song.referenceUrl&&<div className="notes-row-with-delete"><a href={song.referenceUrl} target="_blank" rel="noreferrer">Ouvrir le lien de référence</a>{quickDelete('referenceUrl')}</div>}</section>}
  </div>
  ...
  {fullscreenChoiceOpen&&<Modal className="song-fullscreen-mode-modal" title="Ouvrir en plein écran" onClose={()=>setFullscreenChoiceOpen(false)}><p className="muted-copy">Ce morceau appartient à « {setlistContext?.list.name} ». Choisissez le contexte de scène.</p><div className="song-fullscreen-mode-actions"><button type="button" className="secondary" onClick={()=>{setFullscreenChoiceOpen(false);setFullscreenMode('rehearsal')}}><Play/>Répétition</button><button type="button" className="primary" onClick={()=>{setFullscreenChoiceOpen(false);setFullscreenMode('live')}}><Maximize2/>Live</button></div></Modal>}
  {fullscreenMode&&<SetlistStage standalone={fullscreenMode==='standalone'} mode={fullscreenMode==='standalone'?'live':fullscreenMode} list={setlistContext?.list??{id:'standalone-'+song.id,name:song.title,songIds:[song.id],notes:'',createdAt:song.createdAt,updatedAt:song.updatedAt,deletedAt:null}} songs={setlistContext?.songs??[song]} initialSongId={song.id} refresh={setlistContext?refreshSetlists:async()=>{}} refreshSongs={refreshSongs} toast={toast} onClose={()=>setFullscreenMode(null)} onOpenSong={(stageSong,stageMode)=>{setFullscreenMode(null);if(setlistContext&&onStageOpenSong)onStageOpenSong(stageSong,stageMode);else if(stageSong.id===song.id)onEdit()}}/>}</>
}
...
function songReimportPatch(incoming:SongDraft):Partial<SongDraft>{
  const patch:Partial<SongDraft>={}
  const stringFields:(keyof SongDraft)[]=['title','artist','authorComposer','originalKey','personalKey','timeSignature','style','notes','referenceUrl','structure','chords','chordLyrics','instrumentNotes','lyrics']
  for(const key of stringFields){
    const value=incoming[key]
    if(typeof value==='string'&&value.trim())(patch as any)[key]=value
  }
  if(incoming.bpm!==null)patch.bpm=incoming.bpm
  if(incoming.durationSeconds!==null)patch.durationSeconds=incoming.durationSeconds
  if(incoming.capo!==null&&incoming.capo!==undefined)patch.capo=incoming.capo
  if(incoming.tags?.length)patch.tags=incoming.tags
  if(incoming.musicianNotes&&Object.keys(incoming.musicianNotes).length)patch.musicianNotes=incoming.musicianNotes
  patch.source='import'
  return patch
}
...
function RecueilsPage({songs,setlists,refreshSetlists,entryMode,prefill,onImport,onComplete,onViewImported,onEditImported,toast}:{songs:Song[];setlists:Setlist[];refreshSetlists:()=>Promise<void>;entryMode:'tononkira'|null;prefill:{title?:string;artist?:string}|null;onImport:(draft:SongDraft)=>Promise<Song>;onComplete:(id:string,patch:Partial<SongDraft>)=>Promise<void>;onViewImported:(song:Song)=>void;onEditImported:(song:Song)=>void;toast:(s:string,action?:Toast['action'])=>void}) {
  ...
  const [postImportSetlistOpen,setPostImportSetlistOpen]=useState(false)
  const [postImportSetlistId,setPostImportSetlistId]=useState('')
  const [postImportSetlistBusy,setPostImportSetlistBusy]=useState(false)
  ...
  const addImportedSongToSetlist=async()=>{
    if(!importedSong||!postImportSetlistId||postImportSetlistBusy)return
    const list=setlists.find(x=>x.id===postImportSetlistId)
    if(!list)return
    if(list.songIds.includes(importedSong.id)){
      toast(importedSong.title+' est déjà dans '+list.name+'.')
      setPostImportSetlistOpen(false)
      setPostImportSetlistId('')
      return
    }
    setPostImportSetlistBusy(true)
    try{
      await updateSetlist(list.id,{songIds:[...list.songIds,importedSong.id]})
      await refreshSetlists()
      toast(importedSong.title+' ajouté à '+list.name+'.',{label:'Voir la setlist',run:()=>window.dispatchEvent(new CustomEvent('diart:open-setlist',{detail:{id:list.id}}))})
      setPostImportSetlistOpen(false)
      setPostImportSetlistId('')
    }catch{toast('Ajout à la setlist impossible.')}
    finally{setPostImportSetlistBusy(false)}
  }
  ...
  {importedSong&&<Modal className="post-import-modal" title="Morceau enregistré" onClose={()=>{setImportedSong(null);setPostImportSetlistOpen(false);setPostImportSetlistId('')}}><div className="post-import-success"><Check/><div><b>{importedSong.title}</b><span>{importedSong.artist||'Artiste non renseigné'} est prêt dans DI’ART.</span></div></div><div className="post-import-actions icon-only"><button className="primary icon-btn" aria-label="Voir" title="Voir" onClick={()=>{const song=importedSong;setImportedSong(null);onViewImported(song)}}><BookOpen/></button><button className="secondary icon-btn" aria-label="Ajouter à une setlist" title="Ajouter à une setlist" onClick={()=>{setPostImportSetlistId('');setPostImportSetlistOpen(true)}}><ListPlus/></button><button className="secondary icon-btn" aria-label="Plein écran" title="Plein écran" onClick={()=>{setFullscreenImportedSong(importedSong);setImportedSong(null)}}><Maximize2/></button><button className="secondary icon-btn" aria-label="Modifier" title="Modifier" onClick={()=>{const song=importedSong;setImportedSong(null);onEditImported(song)}}><Pencil/></button><button className="secondary icon-btn" aria-label="Nouvel import" title="Nouvel import" onClick={startNewTononkiraImport}><Import/></button></div>{postImportSetlistOpen&&<div className="post-import-setlist-picker"><div className="setlist-choice-list">{setlists.length?setlists.map(list=>{const added=list.songIds.includes(importedSong.id);const chosen=postImportSetlistId===list.id;return <button type="button" key={list.id} className={`setlist-choice ${added?'already-added':''} ${chosen?'selected':''}`} disabled={added||postImportSetlistBusy} onClick={()=>setPostImportSetlistId(list.id)}><ListMusic/><span><b>{list.name}</b><small>{added?'Déjà ajouté':chosen?'Sélectionnée':`${list.songIds.length} morceau${list.songIds.length>1?'x':''}`}</small></span>{added?<Check/>:chosen?<ChevronRight/>:null}</button>}):<p className="muted-copy">Aucune setlist existante.</p>}</div><div className="post-import-setlist-actions"><button type="button" className="secondary" disabled={postImportSetlistBusy} onClick={()=>{setPostImportSetlistOpen(false);setPostImportSetlistId('')}}>Annuler</button><button type="button" className="primary" disabled={!postImportSetlistId||postImportSetlistBusy} onClick={()=>void addImportedSongToSetlist()}><ListPlus/>{postImportSetlistBusy?'Ajout…':'Ajouter'}</button></div></div>}</Modal>}
  {preview&&<Modal className="chordpro-preview-modal" title="Aperçu ChordPro" onClose={()=>setPreview(null)}><div className="chordpro-preview-head"><FileUp/><div><b>{preview.title}</b><small>{preview.artist||'Artiste non renseigné'} · {fileName}</small></div></div><div className="chordpro-preview-metrics">{preview.originalKey&&<div><span>Tonalité</span><b>{preview.originalKey}</b></div>}{preview.bpm!==null&&<div><span>BPM</span><b>{preview.bpm}</b></div>}{preview.capo!==null&&preview.capo!==undefined&&<div><span>Capo</span><b>{preview.capo}</b></div>}<div><span>Paroles</span><b>{preview.lyrics?.split('\n').filter(Boolean).length??0} lignes</b></div><div><span>Accords</span><b>{preview.chords?.split('\n').filter(Boolean).length??0} lignes</b></div></div>{duplicate&&<div className="duplicate-warning"><AlertTriangle/><span>Ce morceau existe déjà. DI’ART va proposer uniquement les informations manquantes.</span></div>}<div className="chordpro-preview-body">{preview.lyrics&&<section><h3>Paroles</h3><pre>{preview.lyrics.slice(0,1800)}</pre></section>}{preview.chords&&<section><h3>Accords extraits</h3><pre>{preview.chords.slice(0,1200)}</pre></section>}</div><div className="modal-actions"><button className="secondary" onClick={()=>setPreview(null)}>Annuler</button>{duplicate?<><button className="secondary" onClick={()=>{const patch=songReimportPatch(preview);void onComplete(duplicate.id,patch).then(()=>{const updated={...duplicate,...patch,updatedAt:new Date().toISOString()};setPreview(null);setImportedSong(updated);toast('Morceau réimporté depuis ChordPro.')})}}><RefreshCw/>Réimporter</button>{(()=>{const completion=songCompletion(duplicate,preview);return completion.labels.length?<button className="primary" onClick={()=>void onComplete(duplicate.id,completion.patch).then(()=>{const updated={...duplicate,...completion.patch,updatedAt:new Date().toISOString()};setPreview(null);setImportedSong(updated);toast('Morceau complété depuis ChordPro.')})}><RotateCcw/>Compléter : {completion.labels.join(', ')}</button>:null})()}</>:<button className="primary" onClick={()=>void onImport(preview).then(song=>{setPreview(null);setImportedSong(song);toast('Morceau ChordPro ajouté à DI’ART.')})}><Plus/>Ajouter à DI’ART</button>}</div></Modal>}{fullscreenImportedSong&&<SetlistStage standalone mode="live" list={{id:'import-preview-'+fullscreenImportedSong.id,name:fullscreenImportedSong.title,songIds:[fullscreenImportedSong.id],notes:'',createdAt:fullscreenImportedSong.createdAt,updatedAt:fullscreenImportedSong.updatedAt,deletedAt:null}} songs={[fullscreenImportedSong]} refresh={async()=>{}} toast={toast} onClose={()=>setFullscreenImportedSong(null)} onOpenSong={()=>{const song=fullscreenImportedSong;setFullscreenImportedSong(null);if(song)onEditImported(song)}}/>}</>
}
...
function ExternalRecueilSearch({source,badge,name,description,prefill,songs,onImport,onComplete,onImported,toast}:{source:ExternalRecueilSource;badge:string;name:string;description:string;prefill?:{title?:string;artist?:string}|null;songs:Song[];onImport:(draft:SongDraft)=>Promise<Song>;onComplete:(id:string,patch:Partial<SongDraft>)=>Promise<void>;onImported:(song:Song)=>void;toast:(s:string)=>void}) {
  ...
  const run=async()=>{
    ...
    try{
      ...
      const refined=source==='acoustic-gasy'&&artist.trim()
        ? [...found].sort((a,b)=>{const q=normalizeIdentity(artist);const am=normalizeIdentity(a.artist).includes(q)?1:0;const bm=normalizeIdentity(b.artist).includes(q)?1:0;return bm-am||a.title.localeCompare(b.title,'fr',{sensitivity:'base'})})
        : found
      setResults(refined)
...
function HistoryPage({songs,onRestore}:{songs:Song[];onRestore:(item:ActivityEntry)=>Promise<boolean>}) {
  ...
  const restoreItem=async(item:ActivityEntry)=>{if(restoringId)return;setRestoringId(item.id);try{const ok=await onRestore(item);if(ok)setItems(prev=>prev.map(x=>x.id===item.id?{...x,restoredAt:new Date().toISOString()}:x))}finally{setRestoringId('')}}
...
function SetlistsPage({songs,setlists,refresh,refreshSongs,toast,onOpenDetail,onOpenSong}:{songs:Song[];setlists:Setlist[];refresh:()=>Promise<void>;refreshSongs:()=>Promise<void>;toast:(s:string,action?:Toast['action'])=>void;onOpenDetail:(id:string)=>void;onOpenSong:(s:Song,context?:{list:Setlist;mode:'rehearsal'|'live'})=>void}) {
  ...
  {stage&&<SetlistStage mode={stage.mode} list={stage.list} songs={stage.list.songIds.map(id=>songs.find(s=>s.id===id)).filter(Boolean) as Song[]} refresh={refresh} refreshSongs={refreshSongs} toast={toast} onClose={()=>setStage(null)} onOpenSong={(s,mode)=>onOpenSong(s,{list:stage.list,mode})}/>} 
  </>
}
...
function SetlistDetailPage({list,songs,refresh,refreshSongs,toast,restoreY,onRememberPosition,onBack,onOpenSong}:{list:Setlist;songs:Song[];refresh:()=>Promise<void>;refreshSongs:()=>Promise<void>;toast:(s:string)=>void;restoreY:number;onRememberPosition:(y:number)=>void;onBack:()=>void;onOpenSong:(s:Song,stageMode?:'rehearsal'|'live')=>void}) {
  ...
  const [positionEdit,setPositionEdit]=useState<{songId:string;value:string}|null>(null)
  const orderRef=useRef<string[]>(orderIds)
  const currentSetlistScroll=()=>window.scrollY||document.documentElement.scrollTop||0
  const rememberPosition=()=>{const y=currentSetlistScroll();onRememberPosition(y);return y}
  const restorePosition=(y:number)=>requestAnimationFrame(()=>window.scrollTo({top:y,behavior:'auto'}))

  useEffect(()=>{orderRef.current=orderIds},[orderIds])
  useEffect(()=>{setOrderIds([...list.songIds])},[list.songIds])
  useEffect(()=>{const id=requestAnimationFrame(()=>window.scrollTo({top:restoreY,behavior:'auto'}));return()=>cancelAnimationFrame(id)},[])
  ...
  const persistOrder=async(ids:string[])=>{const y=rememberPosition();setOrderIds(ids);orderRef.current=ids;await updateSetlist(list.id,{songIds:ids});await refresh();restorePosition(y)}
  ...
  const move=async(songId:string,dir:-1|1)=>{const ids=[...orderRef.current];const index=ids.indexOf(songId);if(index<0)return;const target=index+dir;if(target<0||target>=ids.length)return;[ids[index],ids[target]]=[ids[target],ids[index]];await persistOrder(ids)}
  const moveToPosition=async(songId:string,rawPosition:string)=>{const ids=[...orderRef.current];const current=ids.indexOf(songId);if(current<0)return;const requested=Math.trunc(Number(rawPosition));if(!Number.isFinite(requested)){setPositionEdit(null);return}const position=Math.max(1,Math.min(ids.length,requested));if(position===current+1){setPositionEdit(null);return}ids.splice(current,1);ids.splice(position-1,0,songId);setPositionEdit(null);await persistOrder(ids);toast(`Morceau déplacé en position ${position}.`)}
  ...
  {stage&&<SetlistStage mode={stage} list={list} songs={listSongs} refresh={refresh} refreshSongs={refreshSongs} toast={toast} onClose={()=>setStage(null)} onOpenSong={onOpenSong}/>}
  </>
}

function SetlistStage({mode,list,songs,refresh,refreshSongs,toast,onClose,onOpenSong,standalone=false,initialSongId}:{mode:'rehearsal'|'live';list:Setlist;songs:Song[];refresh:()=>Promise<void>;refreshSongs?:()=>Promise<void>;toast:(s:string)=>void;onClose:()=>void;onOpenSong:(s:Song,mode:'rehearsal'|'live')=>void;standalone?:boolean;initialSongId?:string}) {
  ...
  const initialIndex=initialSongId?Math.max(0,orderedSongs.findIndex(s=>s.id===initialSongId)):0
  const [index,setIndex]=useState(initialIndex)
  const [view,setView]=useState<'guide'|'lyrics'|'both'>('guide')
  ...
  const [transpose,setTranspose]=useState(()=>{const first=orderedSongs[initialIndex]??orderedSongs[0];return first?setlistSongSavedTranspose(list,first):0})
  ...
  const [stageEdit,setStageEdit]=useState<{field:'structure'|'chords'|'chordLyrics'|'instrumentNotes'|'lyrics'|'musicianNotes';label:string;value:string;role?:string}|null>(null)
  ...
  const effectiveStructure=song?(currentOverride.structure??song.structure??''):''
  const effectiveChords=song?(currentOverride.chords??song.chords??''):''
  const effectiveChordLyrics=song?(currentOverride.chordLyrics??song.chordLyrics??''):''
  const effectiveInstrumentNotes=song?(currentOverride.instrumentNotes??song.instrumentNotes??''):''
  const effectiveLyrics=song?(currentOverride.lyrics??song.lyrics??''):''
  const effectiveMusicianNotes:Record<string,string>={...(song?.musicianNotes??{}),...(currentOverride.musicianNotes??{})}
  ...
  useEffect(()=>{
    if(standalone||initialSongId)return
    try{
  ...
  const structureParts=useMemo(()=>parseStructureSequence(effectiveStructure),[effectiveStructure])
  const rawChordSections=useMemo(()=>chordGuideSections(effectiveChords),[effectiveChords])
  const hasGuide=Boolean(structureParts.length||rawChordSections.length||effectiveInstrumentNotes.trim()||Object.values(effectiveMusicianNotes).some(Boolean))
  ...
  useEffect(()=>{
    const onKey=(e:KeyboardEvent)=>{
      ...
      if(e.key==='Tab'&&hasGuide&&Boolean(effectiveLyrics)){
        e.preventDefault()
        setAutoScroll(false)
        setView(v=>v==='guide'?'lyrics':v==='lyrics'&&effectiveChordLyrics.trim()?'both':'guide')
        requestAnimationFrame(()=>contentRef.current?.scrollTo({top:0,behavior:'smooth'}))
        showStageFeedback(view==='guide'?'Paroles':view==='lyrics'&&effectiveChordLyrics.trim()?'Accords + paroles':'Repères')
        return
      }
      if(e.code==='Space'){
        e.preventDefault()
  ...
  },[locked,hasGuide,song?.id,effectiveLyrics,effectiveChordLyrics,view])

  useEffect(()=>{
    if(!song)return
    setView(hasGuide?'guide':effectiveLyrics?'lyrics':'guide')
  ...
  const openStageContentEditor=(field:'structure'|'chords'|'chordLyrics'|'instrumentNotes'|'lyrics'|'musicianNotes',label:string,value:string,role?:string)=>{
  ...
  const saveStageContentEdit=async(scope:'setlist'|'song')=>{
    if(!stageEdit||!song)return
    const value=stageEdit.value
    if(scope==='setlist'){
      const current=localOverridesRef.current
      const songOverride:any={...(current[song.id]??{})}
      if(stageEdit.field==='musicianNotes'){
        const role=stageEdit.role||musicianRole
        songOverride.musicianNotes={...(songOverride.musicianNotes??{}),[role]:value}
      }else songOverride[stageEdit.field]=value
      const nextOverrides={...current,[song.id]:songOverride}
      ...
      await refresh()
      toast('Modification conservée uniquement pour cette setlist.')
    }else{
      const patch:Partial<SongDraft>={}
      if(stageEdit.field==='musicianNotes'){
        const role=stageEdit.role||musicianRole
        patch.musicianNotes={...(song.musicianNotes??{}),[role]:value}
      }else (patch as any)[stageEdit.field]=value
      await updateSong(song.id,patch)
      const current=localOverridesRef.current
      const songOverride:any={...(current[song.id]??{})}
      if(stageEdit.field==='musicianNotes'){
        const role=stageEdit.role||musicianRole
        const musicianNotes={...(songOverride.musicianNotes??{})}
        delete musicianNotes[role]
        if(Object.keys(musicianNotes).length)songOverride.musicianNotes=musicianNotes
        else delete songOverride.musicianNotes
      }else delete songOverride[stageEdit.field]
      const nextOverrides={...current}
      if(Object.keys(songOverride).length)nextOverrides[song.id]=songOverride
      else delete nextOverrides[song.id]
      localOverridesRef.current=nextOverrides
      setLocalOverrides(nextOverrides)
      await updateSetlist(list.id,{songOverrides:nextOverrides})
      await Promise.all([refresh(),refreshSongs?.()??Promise.resolve()])
      await logActivity('update','Morceau modifié en répétition',stageEdit.label,{songId:song.id,songTitle:song.title,source:'Répétition',setlistId:list.id,setlistName:list.name})
      toast('Modification enregistrée dans la fiche du morceau.')
    }
    setStageEdit(null)
    setStageEditChoosing(false)
  }
  ...
  const displayChordLyrics=transposeChordLyricsText(effectiveChordLyrics,transpose)
  ...
  return createPortal(<div className={'stage-mode '+mode+' stage-theme-'+stageTheme+' stage-role-'+stageRole+(locked?' stage-locked':'')} data-stage-theme={stageTheme} style={stageStyle}>
    <header ref={stageHeaderRef} className="stage-topbar" style={{zIndex:4,background:'rgba(2,9,12,.96)',borderBottom:'1px solid #17323a',display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center'}}>
      ...
      <div className={'stage-current-song '+(showHeaderIdentity?'identity-visible':'identity-hidden')} style={{gridColumn:2,justifySelf:'center',textAlign:'center'}}>
        ...
        {(hasGuide||effectiveLyrics||effectiveChordLyrics.trim())&&<div className="stage-view-tabs">{hasGuide&&<button type="button" className={view==='guide'?'active':''} onClick={()=>{setView('guide');setAutoScroll(false);requestAnimationFrame(()=>contentRef.current?.scrollTo({top:0}))}}>Repères</button>}{effectiveLyrics&&<button type="button" className={view==='lyrics'?'active':''} onClick={()=>{setView('lyrics');setAutoScroll(false);requestAnimationFrame(()=>contentRef.current?.scrollTo({top:0}))}}>Paroles</button>}{effectiveChordLyrics.trim()&&<button type="button" className={view==='both'?'active':''} title="Accords + paroles" aria-label="Accords + paroles" onClick={()=>{setView('both');setAutoScroll(false);requestAnimationFrame(()=>contentRef.current?.scrollTo({top:0}))}}>&amp;</button>}</div>}
      </div>
    ...
    <main key={song.id} ref={contentRef} className={'stage-content stage-song-motion '+(navDirection>0?'motion-next':'motion-prev')} onScroll={handleStageScroll} onWheel={()=>{if(autoScroll){setAutoScroll(false);showStageFeedback('Défilement manuel · pause')}}} onClick={handleTap} onTouchStart={e=>{beginSwipe(e);beginLongPress()}} onTouchMove={e=>{moveSwipe(e);cancelLongPress()}} onTouchEnd={e=>{endSwipe(e);cancelLongPress()}} onTouchCancel={()=>{cancelLongPress();scheduleTouchAutoScrollResume()}} style={{minHeight:0,height:'100%',overflowY:'auto',overflowX:'hidden',WebkitOverflowScrolling:'touch',overscrollBehavior:'contain',touchAction:'pan-y'}}>
      ...
      {view==='both'&&displayChordLyrics.trim()
        ...
        :hasGuide?<div className="stage-guide stage-guide-modern">
          {structureParts.length>0&&<section className={'stage-structure-card '+(mode==='rehearsal'&&!locked?'rehearsal-click-edit':'')} onClick={e=>{if(mode==='rehearsal'&&!locked){e.stopPropagation();openStageContentEditor('structure','Structure',effectiveStructure)}}}><h3>Structure{mode==='rehearsal'&&!locked&&<Pencil/>}</h3><div className="stage-structure-flow">{structureParts.map((part,i)=><span key={part+'-'+i}>{part}</span>)}</div></section>}
          {chordSections.length>0&&<section className={'stage-chords-card '+(mode==='rehearsal'&&!locked?'rehearsal-click-edit':'')} onClick={e=>{if(mode==='rehearsal'&&!locked){e.stopPropagation();openStageContentEditor('chords','Accords / repères',effectiveChords)}}}><h3>Accords / repères{transpose!==0&&<small className="transpose-indicator"> · {formatSemitoneOffset(transpose)} demi-ton{Math.abs(transpose)>1?'s':''}</small>}{mode==='rehearsal'&&!locked&&<Pencil/>}</h3><div className="stage-chord-sections">{chordSections.map((section,i)=><article key={section.label+'-'+i}><b>{section.label}</b><pre>{section.body}</pre></article>)}</div></section>}
          {effectiveInstrumentNotes.trim()&&stageRole!=='musicien'&&<section className={'stage-instrument-card '+(mode==='rehearsal'&&!locked?'rehearsal-click-edit':'')} onClick={e=>{if(mode==='rehearsal'&&!locked){e.stopPropagation();openStageContentEditor('instrumentNotes','Notes instrumentales',effectiveInstrumentNotes)}}}><h3>Notes instrumentales{mode==='rehearsal'&&!locked&&<Pencil/>}</h3><p>{effectiveInstrumentNotes}</p></section>}{stageRole==='musicien'&&<section className={'stage-instrument-card stage-role-note '+(mode==='rehearsal'&&!locked?'rehearsal-click-edit':'')} onClick={e=>{if(mode==='rehearsal'&&!locked){e.stopPropagation();openStageContentEditor('musicianNotes',musicianRole,effectiveMusicianNotes[musicianRole]?.trim()||effectiveInstrumentNotes,musicianRole)}}}><h3>{musicianRole}{mode==='rehearsal'&&!locked&&<Pencil/>}</h3><p>{effectiveMusicianNotes[musicianRole]?.trim()||effectiveInstrumentNotes.trim()||'Aucune note spécifique pour ce rôle.'}</p></section>}
        </div>:null}

      {mode==='rehearsal'&&!locked&&<section className="rehearsal-edit-panel"><div className="panel-title-row"><div><h3>Répétition · points à résoudre</h3><small>Les points non résolus réapparaîtront à la prochaine répétition de ce morceau.</small></div><button type="button" className="secondary" onClick={()=>void saveSetlistChoices().then(()=>{onClose();onOpenSong(song,mode)})}><Pencil/>Modifier la fiche</button></div><div className="rehearsal-issues">{(localIssues[song.id]??[]).length?<>{(localIssues[song.id]??[]).filter(issue=>!issue.resolvedAt).map(issue=><div className="rehearsal-issue" key={issue.id}><button className="issue-check" title="Marquer comme résolu" onClick={()=>void toggleIssue(issue.id)}><span/></button><span><b>{issue.text}</b><small>Ajouté {new Date(issue.createdAt).toLocaleString()}</small></span><button className="bare-action danger-icon" onClick={()=>void removeIssue(issue.id)}><Trash2/></button></div>)}{(localIssues[song.id]??[]).some(issue=>issue.resolvedAt)&&<details className="resolved-issues"><summary>Points résolus · {(localIssues[song.id]??[]).filter(issue=>issue.resolvedAt).length}</summary>{(localIssues[song.id]??[]).filter(issue=>issue.resolvedAt).map(issue=><div className="rehearsal-issue resolved" key={issue.id}><button className="issue-check" title="Rouvrir" onClick={()=>void toggleIssue(issue.id)}><Check/></button><span><b>{issue.text}</b><small>Résolu {new Date(issue.resolvedAt!).toLocaleString()}</small></span><button className="bare-action danger-icon" onClick={()=>void removeIssue(issue.id)}><Trash2/></button></div>)}</details>}</>:<p className="muted-copy">Aucun point à revoir pour ce morceau.</p>}</div><div className="issue-add-row"><input value={issueDraft} onChange={e=>setIssueDraft(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();void addIssue()}}} placeholder="Ex. Refaire la fin, sax mesure 17, ralentir le refrain…"/><button className="primary" onClick={()=>void addIssue()}><Plus/>Ajouter</button></div><details className="rehearsal-general-notes"><summary>Notes générales de répétition</summary><textarea value={noteDraft} onChange={e=>setNoteDraft(e.target.value)} placeholder="Notes libres propres à cette setlist…"/><button type="button" className="secondary" onClick={()=>void saveNote()}><Save/>Enregistrer les notes</button></details></section>}
    </main>
