...
function SongRow({song,onOpen,onFav,action}:{song:Song;onOpen:()=>void;onFav:()=>void;action?:ReactNode}) {
  ...
  return <div className={`song-row ${action?'has-action':''} ${hasMeta?'':'no-meta'}`} onClick={onOpen} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==='Enter')onOpen()}}>
    ...
    <div className="song-main"><b>{song.title}</b><span>{song.artist || 'Artiste inconnu'}{song.source==='demo'&&<em>DEMO</em>}{song.favoriteStatus&&song.favoriteStatus!=='favorite'&&<em className={'personal-status '+song.favoriteStatus}>{FAVORITE_STATUS_OPTIONS.find(([v])=>v===song.favoriteStatus)?.[1]}</em>}</span></div>
