  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      <div className={`content page-transition-surface page-${page}`} key={page+':'+selectedArtist+':'+selectedAuthor+':'+selectedSetlistId+':'+(selected?.id??'')}>
        ...
        {page==='setlist'&&currentSetlist&&<SetlistDetailPage list={currentSetlist} songs={songs} refresh={refreshSetlists} refreshSongs={refresh} toast={toast} onBack={()=>go('setlists')} onOpenSong={(s,stageMode)=>openSong(s,{page:'setlist',label:currentSetlist.name,stageMode,setlist:currentSetlist})}/>} 
...
function SetlistDetailPage({list,songs,refresh,refreshSongs,toast,onBack,onOpenSong}:{list:Setlist;songs:Song[];refresh:()=>Promise<void>;refreshSongs:()=>Promise<void>;toast:(s:string)=>void;onBack:()=>void;onOpenSong:(s:Song,stageMode?:'rehearsal'|'live')=>void}) {
