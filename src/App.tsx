...
function SetlistsPage({songs,setlists,refresh,toast,onOpenDetail}:{songs:Song[];setlists:Setlist[];refresh:()=>Promise<void>;toast:(s:string,action?:Toast['action'])=>void;onOpenDetail:(id:string)=>void}) {
  ...
  {stage&&<SetlistStage mode={stage.mode} list={stage.list} songs={stage.list.songIds.map(id=>songs.find(s=>s.id===id)).filter(Boolean) as Song[]} refresh={refresh} toast={toast} onClose={()=>setStage(null)} onOpenSong={()=>{}}/>}
  </>
}
