...
import {
  ...
  Filter, ArrowUpDown, Check, AlertTriangle
} from 'lucide-react'
...
import { emptySongDraft, formatDuration, normalizeKey, parseBpm, parseDuration, searchSong } from './music'
...
function SongDetail({song,onBack,onEdit,onFav,onDelete}:{song:Song;onBack:()=>void;onEdit:()=>void;onFav:()=>void;onDelete:()=>void}) {
  ...
  return <><div className="detail-nav"><button className="ghost" onClick={onBack}><ChevronLeft/>Bibliothèque</button><div><button className="icon-btn" onClick={onFav}><Heart fill={song.favorite?'currentColor':'none'}/></button><button className="secondary" onClick={onEdit}><Pencil/>Modifier</button><button className="danger" onClick={()=>setConfirm(true)}><Trash2/>Supprimer</button></div></div>
  <section className="song-hero"><div><p className="eyebrow">{song.style||'Morceau'}{song.source==='demo'?' · DEMO':''}</p><h1>{song.title}</h1><p>{song.artist||'Artiste inconnu'}{song.authorComposer?` · ${song.authorComposer}`:''}</p></div><div className="key-bpm"><div><span>Tonalité</span><strong>{song.personalKey||song.originalKey||'—'}</strong></div><div><span>BPM</span><strong>{song.bpm??'—'}</strong></div><div><span>Signature</span><strong>{song.timeSignature||'—'}</strong></div></div></section>
  <div className="two-col"><section className="panel info-list"><h2>Informations</h2><div><span>Tonalité originale</span><b>{song.originalKey||'—'}</b></div><div><span>Tonalité habituelle</span><b>{song.personalKey||'—'}</b></div><div><span>Durée</span><b>{formatDuration(song.durationSeconds)}</b></div><div><span>Tags</span><b>{song.tags.join(', ')||'—'}</b></div></section><section className="panel"><h2>Notes</h2><p className="notes">{song.notes||'Aucune note.'}</p>{song.referenceUrl&&<a href={song.referenceUrl} target="_blank" rel="noreferrer">Ouvrir le lien de référence</a>}</section></div>
