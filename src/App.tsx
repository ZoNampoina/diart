...
function SongDetail({song,backLabel,setlists,refreshSetlists,toast,onBack,onEdit,onQuickDelete,onArtist,onFav,onLyricsSave,onRecueilSearch,onDelete,setlistKey,onSetlistKey}:{song:Song;backLabel:string;setlists:Setlist[];refreshSetlists:()=>Promise<void>;toast:(s:string)=>void;onBack:()=>void;onEdit:()=>void;onQuickDelete:(field:QuickDeleteField)=>void;onArtist:(name:string)=>void;onFav:()=>void;onLyricsSave:(lyrics:string)=>Promise<void>;onRecueilSearch:(prefill:{title:string;artist:string})=>void;onDelete:()=>void;setlistKey?:string;onSetlistKey?:(key:string)=>void}) {
