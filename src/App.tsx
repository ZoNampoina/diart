...
function SetlistStage({mode,list,songs,refresh,toast,onClose,onOpenSong,standalone=false}:{mode:'rehearsal'|'live';list:Setlist;songs:Song[];refresh:()=>Promise<void>;toast:(s:string)=>void;onClose:()=>void;onOpenSong:(s:Song)=>void;standalone?:boolean}) {
  ...
  const stageTools=<div className={'stage-session-tools '+(toolsCollapsed?'collapsed':'')}>
    ...
    <div className={'stage-tools-body collapsible-body '+(toolsCollapsed?'is-collapsed':'is-expanded')}>
      ...
      <div className="stage-control-group"><span>Transposer</span><button type="button" title="-1 demi-ton" onClick={()=>persistTranspose(Math.max(-12,transpose-1))}><Minus/></button><b>{formatSemitoneOffset(transpose)}</b><button type="button" title="+1 demi-ton" onClick={()=>persistTranspose(Math.min(12,transpose+1))}><Plus/></button><button type="button" title="Réinitialiser" className="stage-reset-btn" disabled={transpose===0} onClick={()=>persistTranspose(0)}><RotateCcw/></button></div>
