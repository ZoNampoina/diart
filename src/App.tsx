...
function LibraryPage({songs,setlists,refreshSetlists,toast,searchRef,onOpen,onFav,onDeleteMany,onMerge,onRecueilSearch}:{songs:Song[];setlists:Setlist[];refreshSetlists:()=>Promise<void>;toast:(s:string)=>void;searchRef:RefObject<HTMLInputElement>;onOpen:(s:Song)=>void;onFav:(s:Song)=>void;onDeleteMany:(songs:Song[])=>Promise<void>;onMerge:(primary:Song,secondary:Song)=>Promise<void>;onRecueilSearch:(query:string)=>void}) {
  ...
  return <>
    <div className="page-head compact library-page-head"><div><p className="eyebrow">Bibliothèque</p><h1>{songs.length} morceaux</h1></div><button className={manage?'primary':'secondary'} onClick={()=>manage?exitManage():setManage(true)}>{manage?<X/>:<MoreHorizontal/>}{manage?'Terminer':'Gérer'}</button></div>
    <div className="toolbar library-toolbar"><div className="searchbox"><Search/><input ref={searchRef} value={q} onChange={e=>setQ(e.target.value)} placeholder="Titre, artiste, auteur, tonalité, BPM, tags…"/>{q&&<button type="button" className="search-clear" aria-label="Effacer la recherche" onClick={()=>setQ('')}><X/></button>}</div><button className={'icon-btn library-tool-btn '+(filters?'active':'')} aria-label="Filtres" title="Filtres" onClick={()=>{setFilters(v=>!v);setSortOpen(false)}}><Filter/></button><button className={'icon-btn library-tool-btn '+(sortOpen?'active':'')} aria-label="Trier" title="Trier" onClick={()=>{setSortOpen(v=>!v);setFilters(false)}}><ArrowUpDown/></button></div>
    {sortOpen&&<div className="library-sort-popover"><ArrowUpDown/><select aria-label="Ordre de tri" value={sort} onChange={e=>{setSort(e.target.value);setSortOpen(false)}}><option value="title">Titre A–Z</option><option value="artist">Artiste A–Z</option><option value="bpm">BPM</option><option value="updated">Modifiés récemment</option></select></div>}
...
function RecueilsPage({songs,entryMode,prefill,onImport,onComplete,onViewImported,onEditImported,toast}:{songs:Song[];entryMode:'tononkira'|null;prefill:{title?:string;artist?:string}|null;onImport:(draft:SongDraft)=>Promise<Song>;onComplete:(id:string,patch:Partial<SongDraft>)=>Promise<void>;onViewImported:(song:Song)=>void;onEditImported:(song:Song)=>void;toast:(s:string)=>void}) {
  ...
  useEffect(()=>{if(entryMode!=='tononkira')return;setTononkiraTitle(prefill?.title??'');setTononkiraArtist(prefill?.artist??'');setTononkiraResults([]);setTononkiraSearched(false);requestAnimationFrame(()=>tononkiraTitleRef.current?.focus())},[entryMode,prefill?.title,prefill?.artist])
