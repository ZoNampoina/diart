...
function SongDetail({song,backLabel,setlists,refreshSetlists,toast,suppressOpenSetlistId,onBack,onEdit,onQuickDelete,onArtist,onFav,onLyricsSave,onRecueilSearch,onDelete,setlistKey,onSetlistKey}:{song:Song;backLabel:string;setlists:Setlist[];refreshSetlists:()=>Promise<void>;toast:(s:string,action?:Toast['action'])=>void;suppressOpenSetlistId?:string;onBack:()=>void;onEdit:()=>void;onQuickDelete:(field:QuickDeleteField)=>void;onArtist:(name:string)=>void;onFav:()=>void;onLyricsSave:(lyrics:string)=>Promise<void>;onRecueilSearch:(prefill:{title:string;artist:string})=>void;onDelete:()=>void;setlistKey?:string;onSetlistKey?:(key:string)=>void}) {
...
function SetlistsPage({songs,setlists,refresh,refreshSongs,toast,onOpenDetail,onOpenSong}:{songs:Song[];setlists:Setlist[];refresh:()=>Promise<void>;refreshSongs:()=>Promise<void>;toast:(s:string,action?:Toast['action'])=>void;onOpenDetail:(id:string)=>void;onOpenSong:(s:Song)=>void}) {
  ...
  {stage&&<SetlistStage mode={stage.mode} list={stage.list} songs={stage.list.songIds.map(id=>songs.find(s=>s.id===id)).filter(Boolean) as Song[]} refresh={refresh} refreshSongs={refreshSongs} toast={toast} onClose={()=>setStage(null)} onOpenSong={onOpenSong}/>} 
  </>
}

function SetlistDetailPage({list,songs,refresh,refreshSongs,toast,onBack,onOpenSong}:{list:Setlist;songs:Song[];refresh:()=>Promise<void>;refreshSongs:()=>Promise<void>;toast:(s:string)=>void;onBack:()=>void;onOpenSong:(s:Song)=>void}) {
...
function SetlistStage({mode,list,songs,refresh,refreshSongs,toast,onClose,onOpenSong,standalone=false}:{mode:'rehearsal'|'live';list:Setlist;songs:Song[];refresh:()=>Promise<void>;refreshSongs?:()=>Promise<void>;toast:(s:string)=>void;onClose:()=>void;onOpenSong:(s:Song)=>void;standalone?:boolean}) {
