...
function LibraryPage({songs,setlists,refreshSetlists,toast,searchRef,onOpen,onFav,onDeleteMany,onMerge,onRecueilSearch}:{songs:Song[];setlists:Setlist[];refreshSetlists:()=>Promise<void>;toast:(s:string)=>void;searchRef:RefObject<HTMLInputElement>;onOpen:(s:Song)=>void;onFav:(s:Song)=>void;onDeleteMany:(songs:Song[])=>Promise<void>;onMerge:(primary:Song,secondary:Song)=>Promise<void>;onRecueilSearch:(query:string)=>void}) {
  ...
  return <>
    ...
    {filters&&<Modal className="library-tool-modal" title="Filtres" onClose={()=>setFilters(false)}><div className="filters library-filter-modal"><select value={key} onChange={e=>setKey(e.target.value)}><option value="">Toutes tonalités</option>{keys.map(x=><option key={x}>{x}</option>)}</select><div className="filter-bpm-row"><input value={min} onChange={e=>setMin(e.target.value)} placeholder="BPM min"/><input value={max} onChange={e=>setMax(e.target.value)} placeholder="BPM max"/></div><select value={sig} onChange={e=>setSig(e.target.value)}><option value="">Toutes signatures</option>{sigs.map(x=><option key={x}>{x}</option>)}</select><select value={style} onChange={e=>setStyle(e.target.value)}><option value="">Tous styles</option>{styles.map(x=><option key={x}>{x}</option>)}</select><label><input type="checkbox" checked={favOnly} onChange={e=>setFavOnly(e.target.checked)}/> Favoris uniquement</label><div className="modal-actions"><button className="secondary" onClick={()=>{setKey('');setSig('');setStyle('');setFavOnly(false);setMin('');setMax('')}}>Réinitialiser</button><button className="primary" onClick={()=>setFilters(false)}><Check/>Appliquer</button></div></div></Modal>}
