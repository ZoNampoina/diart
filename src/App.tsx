...
function SongDetail({song,setlists,refreshSetlists,toast,onBack,onEdit,onFav,onLyricsSave,onDelete}:{song:Song;setlists:Setlist[];refreshSetlists:()=>Promise<void>;toast:(s:string)=>void;onBack:()=>void;onEdit:()=>void;onFav:()=>void;onLyricsSave:(lyrics:string)=>Promise<void>;onDelete:()=>void}) {
...
function SetlistsPage({songs,setlists,refresh,toast,onOpenDetail}:{songs:Song[];setlists:Setlist[];refresh:()=>Promise<void>;toast:(s:string,action?:Toast['action'])=>void;onOpenDetail:(id:string)=>void}) {
  const [name,setName]=useState('')
  const [open,setOpen]=useState<string|null>(null)
  ...
  return <><div className="page-head"><div><p className="eyebrow">Setlists</p><h1>Setlists</h1><p>Premier clic : aperçu. Deuxième clic : ouvrir la page complète.</p></div><div className="setlist-create"><input value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void create()}} placeholder="Nom de la nouvelle setlist"/><button className="primary" disabled={!name.trim()} onClick={()=>void create()}><Plus/>Créer</button></div></div>
  <div className="setlist-grid modern-setlist-grid">{setlists.length?setlists.map(list=>{
    ...
    return <section className={`panel setlist-card modern-setlist-card ${isOpen?'open':''}`} key={list.id}><div className="setlist-head">{editing===list.id?<div className="setlist-rename"><input autoFocus value={editName} onChange={e=>setEditName(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void rename(list);if(e.key==='Escape')setEditing(null)}}/><button className="icon-btn" onClick={()=>void rename(list)}><Check/></button><button className="icon-btn" onClick={()=>setEditing(null)}><X/></button></div>:<button className="setlist-title-button" onClick={()=>clickList(list)}><span className="setlist-cover"><ListMusic/></span><div><b>{list.name}</b><small>{list.songIds.length} morceau{list.songIds.length>1?'x':''}{isOpen?' · cliquer à nouveau pour ouvrir':''}</small></div><ChevronRight className={isOpen?'rotated':''}/></button>}<div className="setlist-head-actions"><button className="bare-action" aria-label="Renommer" title="Renommer" onClick={()=>{setEditing(list.id);setEditName(list.name)}}><Pencil/></button><button className="bare-action danger-icon" aria-label="Supprimer la setlist" title="Supprimer" onClick={()=>setDeleting(list)}><Trash2/></button></div></div>{isOpen&&<div className="setlist-preview"><div className="setlist-preview-songs">{preview.length?preview.slice(0,4).map((s,i)=><div key={s.id}><span>{i+1}</span><b>{s.title}</b><small>{s.personalKey||s.originalKey||'—'}{s.bpm!==null?` · ${s.bpm} BPM`:''}</small></div>):<p>Aucun morceau pour le moment.</p>}</div><button className="primary open-setlist-btn" onClick={()=>onOpenDetail(list.id)}>Ouvrir la setlist<ChevronRight/></button></div>}</section>
...
function SetlistDetailPage({list,songs,refresh,toast,onBack,onOpenSong}:{list:Setlist;songs:Song[];refresh:()=>Promise<void>;toast:(s:string)=>void;onBack:()=>void;onOpenSong:(s:Song)=>void}) {
  ...
  return <><div className="detail-nav setlist-detail-nav"><button className="ghost" onClick={onBack}><ChevronLeft/>Setlists</button><div className="setlist-mode-actions"><button className="secondary" disabled={!listSongs.length} onClick={()=>setStage('rehearsal')}><Play/>Répétition</button><button className="primary" disabled={!listSongs.length} onClick={()=>setStage('live')}><Maximize2/>Live Mode</button></div></div>
  <section className="setlist-detail-hero"><div><span className="setlist-hero-icon"><ListMusic/></span><p className="eyebrow">Setlist</p><h1>{list.name}</h1><p>{listSongs.length} morceau{listSongs.length>1?'x':''} · {lyricsCount} avec paroles</p></div><div className="setlist-detail-metrics"><div><span>Durée</span><b>{totalSeconds?formatDuration(totalSeconds):'—'}</b></div><div><span>BPM moyen</span><b>{avgBpm??'—'}</b></div><div><span>Paroles</span><b>{lyricsCount}/{listSongs.length}</b></div></div></section>
  <section className="panel setlist-manager"><div className="panel-title-row"><h2>Ordre des morceaux</h2><label className="setlist-add-select"><Plus/><select defaultValue="" onChange={e=>{const id=e.target.value;e.target.value='';if(id)void add(id)}}><option value="">Ajouter un morceau</option>{songs.filter(s=>!list.songIds.includes(s.id)).map(s=><option key={s.id} value={s.id}>{s.title} — {s.artist}</option>)}</select></label></div><div className="setlist-detail-songs">{listSongs.length?listSongs.map((s,i)=><div className="setlist-detail-song" key={s.id}><span className="setlist-number">{i+1}</span><button className="setlist-song-main" onClick={()=>onOpenSong(s)}><b>{s.title}</b><small>{s.artist||'Artiste inconnu'} · {s.personalKey||s.originalKey||'—'}{s.bpm!==null?` · ${s.bpm} BPM`:''}</small></button><div className="setlist-song-flags">{s.lyrics&&<span>Paroles</span>}{list.rehearsalNotes?.[s.id]&&<span>Notes</span>}</div><button className="bare-action" disabled={i===0} aria-label="Monter" onClick={()=>void move(i,-1)}><ChevronUp/></button><button className="bare-action" disabled={i===listSongs.length-1} aria-label="Descendre" onClick={()=>void move(i,1)}><ChevronDown/></button><button className="bare-action danger-icon" aria-label="Retirer" onClick={()=>void remove(s.id)}><X/></button></div>):<Empty text="Cette setlist est vide."/>}</div></section>
...
function SetlistStage({mode,list,songs,refresh,toast,onClose,onOpenSong}:{mode:'rehearsal'|'live';list:Setlist;songs:Song[];refresh:()=>Promise<void>;toast:(s:string)=>void;onClose:()=>void;onOpenSong:(s:Song)=>void}) {
  ...
  useEffect(()=>{if(!song)return;setView('guide');setNoteDraft(localNotes[song.id]??'')},[index,song?.id])
  if(!song)return null
  ...
  return createPortal(<div className={`stage-mode ${mode}`}><header className="stage-topbar"><div><span>{mode==='rehearsal'?'Répétition':'Live Mode'}</span><b>{list.name}</b></div><div className="stage-view-tabs"><button className={view==='guide'?'active':''} onClick={()=>setView('guide')}>Repères</button>{song.lyrics&&<button className={view==='lyrics'?'active':''} onClick={()=>setView('lyrics')}>Paroles</button>}</div><button className="live-close" onClick={onClose}><X/></button></header><main className="stage-content"><div className="stage-song-head"><p>{song.artist||'Artiste inconnu'} · {index+1}/{songs.length}</p><h1>{song.title}</h1><div className="stage-metrics"><strong>{song.personalKey||song.originalKey||'—'}</strong>{song.bpm!==null&&<span>{song.bpm} BPM</span>}{song.timeSignature&&<span>{song.timeSignature}</span>}</div></div>{view==='lyrics'&&song.lyrics?<pre className="stage-lyrics">{song.lyrics}</pre>:<div className="stage-guide">{song.structure&&<section><h3>Structure</h3><p>{song.structure}</p></section>}{song.chords&&<section><h3>Accords / repères</h3><pre>{song.chords}</pre></section>}{song.instrumentNotes&&<section><h3>Notes instrumentales</h3><p>{song.instrumentNotes}</p></section>}{!song.structure&&!song.chords&&!song.instrumentNotes&&<p className="stage-empty">Aucun repère renseigné.</p>}</div>}{mode==='rehearsal'&&<section className="rehearsal-edit-panel"><div className="panel-title-row"><div><h3>Modifications / notes de répétition</h3><small>Ces annotations restent liées à cette setlist et se synchronisent sur vos appareils.</small></div><button className="secondary" onClick={()=>{onClose();onOpenSong(song)}}><Pencil/>Modifier la fiche</button></div><textarea value={noteDraft} onChange={e=>setNoteDraft(e.target.value)} placeholder="Ex. Reprendre le pont 2x, sax après le refrain, descendre d’un ton…"/><button className="primary" onClick={()=>void saveNote()}><Save/>Enregistrer les modifications</button></section>}</main><footer className="stage-nav"><button className="secondary" disabled={index===0} onClick={()=>go(index-1)}><ChevronLeft/>Précédent</button><div>{index+1} / {songs.length}</div><button className="primary" disabled={index===songs.length-1} onClick={()=>go(index+1)}>Suivant<ChevronRight/></button></footer></div>,document.body)
}
