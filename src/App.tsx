...
function SetlistStage({mode,list,songs,refresh,refreshSongs,toast,onClose,onOpenSong,standalone=false,initialSongId}:{mode:'rehearsal'|'live';list:Setlist;songs:Song[];refresh:()=>Promise<void>;refreshSongs?:()=>Promise<void>;toast:(s:string)=>void;onClose:()=>void;onOpenSong:(s:Song,mode:'rehearsal'|'live')=>void;standalone?:boolean;initialSongId?:string}) {
  ...
  useEffect(()=>{
    const onKey=(e:KeyboardEvent)=>{
      ...
      if(e.key==='Tab'&&hasGuide&&Boolean(effectiveLyrics)){
        e.preventDefault()
        setAutoScroll(false)
        setView(v=>v==='guide'?'lyrics':v==='lyrics'&&(effectiveChordLyrics.trim()||(effectiveLyrics&&effectiveChords))?'both':'guide')
        requestAnimationFrame(()=>contentRef.current?.scrollTo({top:0,behavior:'smooth'}))
        showStageFeedback(view==='guide'?'Paroles':view==='lyrics'&&(effectiveChordLyrics.trim()||(effectiveLyrics&&effectiveChords))?'Accords + paroles':'Repères')
        return
      }
      if(e.code==='Space'){
        e.preventDefault()
  ...
  },[locked,hasGuide,song?.id,effectiveLyrics,effectiveChords,effectiveChordLyrics,view])
  ...
  const displayChordLyrics=transposeChordLyricsText(effectiveChordLyrics.trim()?effectiveChordLyrics:combineLyricsAndChords(effectiveLyrics,effectiveChords),transpose)
