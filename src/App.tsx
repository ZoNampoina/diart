...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      <div className="content page-transition-surface" key={page+':'+selectedArtist+':'+selectedAuthor+':'+selectedSetlistId+':'+(selected?.id??'')}>
...
function SetlistsPage({songs,setlists,refresh,toast,onOpenDetail,onOpenSong}:{songs:Song[];setlists:Setlist[];refresh:()=>Promise<void>;toast:(s:string,action?:Toast['action'])=>void;onOpenDetail:(id:string)=>void;onOpenSong:(s:Song)=>void}) {
  ...
  return <><div className="setlist-create setlist-create-standalone"><input className="new-setlist-name-input" value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')void create()}} placeholder="Nom de la nouvelle setlist"/><button className="primary" disabled={!name.trim()} onClick={()=>void create()}><Plus/>Créer</button></div>
