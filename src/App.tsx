...
function SongForm({initial,songs,presetArtist='',presetAuthor='',onCancel,onSave,onMergeDuplicate}:{initial:Song|null;songs:Song[];presetArtist?:string;presetAuthor?:string;onCancel:()=>void;onSave:(d:SongDraft)=>Promise<void>;onMergeDuplicate:(d:SongDraft,duplicate:Song)=>Promise<void>}) {
  ...
  const tagSuggestions=useMemo(()=>tagFragment?allTags.filter(t=>normalizeIdentity(t).startsWith(normalizeIdentity(tagFragment))&&!d.tags.slice(0,-1).some(x=>normalizeIdentity(x)===normalizeIdentity(t))).slice(0,6):allTags.filter(t=>!d.tags.includes(t)).slice(0,6),[allTags,d.tags,tagFragment])
  const addSuggestedTag=(tag:string)=>{const next=[...d.tags];if(next.length&&tagFragment)next[next.length-1]=tag;else next.push(tag);set('tags',[...new Set(next.map(x=>x.trim()).filter(Boolean))])}
