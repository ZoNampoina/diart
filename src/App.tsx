...
function SetlistsPage({songs,setlists,refresh,toast}:{songs:Song[];setlists:Setlist[];refresh:()=>Promise<void>;toast:(s:string,action?:Toast['action'])=>void}) {
  const [name,setName]=useState('')
  const [open,setOpen]=useState<string|null>(null)
  ...
  const deleteList=async(list:Setlist)=>{const deletedAt=new Date().toISOString();await updateSetlist(list.id,{deletedAt});await refresh();setDeleting(null);toast(`Setlist « ${list.name} » supprimée.`,{label:'Annuler',run:async()=>{await updateSetlist(list.id,{deletedAt:null});await refresh()}})}
