...
function SongListKey({song}:{song:Song}){
  ...
  return <>{info.value}{info.habitual&&<sup className="habitual-key-mark" title="Tonalité habituelle · tonalité originale non renseignée">H</sup>}</>
}

function SongRow({song,onOpen,onFav,action,showLyricsIndicator=false}:{song:Song;onOpen:()=>void;onFav:()=>void;action?:ReactNode;showLyricsIndicator?:boolean}) {
  ...
  return <div className={`song-row ${action?'has-action':''} ${hasMeta?'':'no-meta'}`} onClick={onOpen} role="button" tabIndex={0} onKeyDown={e=>{if(e.key==='Enter')onOpen()}}>
    ...
    <div className="song-main"><b>{song.title}</b><span>{song.artist || 'Artiste inconnu'}{song.source==='demo'&&<em>DEMO</em>}{song.favoriteStatus&&song.favoriteStatus!=='favorite'&&<em className={'personal-status '+song.favoriteStatus}>{FAVORITE_STATUS_OPTIONS.find(([v])=>v===song.favoriteStatus)?.[1]}</em>}{showLyricsIndicator&&hasLyrics&&<em className="lyrics-available" title="Paroles disponibles"><BookOpen size={12}/>Paroles</em>}</span></div>
...
function LibraryPage({songs,setlists,refreshSetlists,toast,searchRef,onOpen,onFav,onDeleteMany,onMerge,onRecueilSearch}:{songs:Song[];setlists:Setlist[];refreshSetlists:()=>Promise<void>;toast:(s:string)=>void;searchRef:RefObject<HTMLInputElement>;onOpen:(s:Song)=>void;onFav:(s:Song)=>void;onDeleteMany:(songs:Song[])=>Promise<void>;onMerge:(primary:Song,secondary:Song,draft?:SongDraft)=>Promise<void>;onRecueilSearch:(query:string)=>void}) {
  ...
  const result=useMemo(()=>songs.filter(s=>searchSong(s,deferredQ))
    ...
    .filter(s=>!hasLyrics||Boolean(s.lyrics?.trim())).filter(s=>!hasChords||hasMeaningfulChordContent(s.chords??''))
  ...
  [songs,deferredQ,key,sig,style,favOnly,min,max,sort,artistFilter,authorFilter,tagFilter,favoriteStatusFilter,hasLyrics,hasChords])
...
function detectTononkiraStructure(lyrics:string):TononkiraStructureBlock[]{
  ...
  return blocks.map((text,index)=>{
    ...
    if(kind==='refrain')refrainNo++; else if(kind==='verse')verseNo++
