...
function SetlistStage({mode,list,songs,refresh,refreshSongs,toast,onClose,onOpenSong,standalone=false,initialSongId}:{mode:'rehearsal'|'live';list:Setlist;songs:Song[];refresh:()=>Promise<void>;refreshSongs?:()=>Promise<void>;toast:(s:string)=>void;onClose:()=>void;onOpenSong:(s:Song,mode:'rehearsal'|'live')=>void;standalone?:boolean;initialSongId?:string}) {
  ...
  },[locked,hasGuide,song?.id,effectiveLyrics,view])
